...
export default function ModelCenterClient({ initialPredictions, initialStatus, initialBacktest }) {
  const predictions = usePitWallData("/api/predictions", { initialData: initialPredictions });
  const status = usePitWallData("/api/model-status", { initialData: initialStatus });
  const backtest = usePitWallData("/api/backtest", { initialData: initialBacktest });
